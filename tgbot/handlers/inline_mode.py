        ...
        results=[
            InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    message_text=f"https://t.me/QuranMemorizerBot?start={query.from_user.id}"),
            )
        ], cache_time=1000
    )
...
@inline_mode_router.inline_query(F.query == "chapters")
async def search_chapter(inline_query: InlineQuery):
    results = []
    offset = int(inline_query.offset) if inline_query.offset else 0
    all_items = await cmc_client.get_chapters(offset)
    for item in all_items:
        url = f'quran.com/{item.get("id")}'
        thumb_url: str = "https://www.arabiantongue.com/wp-content/uploads/2023/12/i-2023-12-07T211245.728-1.webp"
        title = item.get("name_simple")
        result = InlineQueryResultArticle(
            id=str(item.get("id") * random.randint(1, 1_000_000)),
            title=title,
            input_message_content=InputTextMessageContent(message_text=url),
            description=url,
            thumb_url=thumb_url,
...
# @inline_mode_router.inline_query(F.query.contains('chapters'))
# async def search_friend(inline_query: InlineQuery):
#     query = inline_query.query.split('chapters ')[1]
#     results = []
#     offset = int(inline_query.offset) if inline_query.offset else 0
#     all_items = await cmc_client.get_chapters(offset, q=query)
#     for item in all_items:
#         url = f'https://t.me/{item.get("username")}'
#         thumb_url: str = "https://i.pinimg.com/736x/2d/75/50/2d7550d033be4f1f61b27bce357f6ff0.jpg"
#         result = InlineQueryResultArticle(
